...
import Controller.MaterialControllerAC;
import Controller.VoltageDropControllerAC;
import Controller.PhiCosControllerAC;
 ...
 */
public class JPanelMaterial_VoltageDropAC extends JPanel
{
    //Frame object.
    ...
    //Fields of the class.
    //Material
    private JComboBox<String> materialJComboBox;
    ...
    //Phi Cos.
    private JTextField phiCosJTextField;
    private String initialPhiCos=null;
    private Double finalPhiCos=null;
     ...
     */
    JPanelMaterial_VoltageDropAC(FrameVoltageDrop objectFrame)
    {
        ...
        DecimalFormat PhiCosFormat=new DecimalFormat("#.##",symbol); //Phi Cos.
        ...
        materialJComboBox.addItemListener(itemEvent ->
                {
                    if(!init)
                    {
                        ...
                        MaterialControllerAC materialControllerAC =new MaterialControllerAC(frame);
                        materialControllerAC.updateFrame();
        ...
        VoltageDropTittle.setBounds(new Rectangle(10,134,120,20));
        ...
        JLabel voltageDropJLabel=new JLabel("Voltage drop (AC):");
        voltageDropJLabel.setForeground(Color.LIGHT_GRAY);
        voltageDropJLabel.setBounds(new Rectangle(60,160,160,20));
        ...
        voltageDropJTextField.setBounds(new Rectangle(200,160,60,20));
        ...
        //************************************************************************************
        //ADDING THE PHI COS TO THE VOLTAGE DROP PANEL.
        //************************************************************************************

        //Cos phi label for the text field.
        JLabel phiCosJLabel=new JLabel("Phi cos or power factor:");
        phiCosJLabel.setForeground(Color.LIGHT_GRAY);
        phiCosJLabel.setBounds(new Rectangle(35,196,190,20));

        //Text field for the phi cos.
        phiCosJTextField=new JTextField();
        phiCosJTextField.setEditable(true);
        phiCosJTextField.setHorizontalAlignment(JTextField.RIGHT);
        phiCosJTextField.setColumns(6);
        phiCosJTextField.setBounds(new Rectangle(220,196,60,20));
        phiCosJTextField.setToolTipText("Enter the phi cos or power factor, range: > 0,8 to 1");

        //Add the method to verify the value of the text field before to lost the focus.
        phiCosJTextField.setInputVerifier(new InputVerifier()
                                               {
                                                   @Override
                                                   public boolean verify(JComponent jComponent)
                                                   {
            ...
            {
                //Parse throws a exception if there is some error to catching the value.
                finalPhiCos=PhiCosFormat.parse(phiCosJTextField.getText()).doubleValue();

                //If the format is correct and the value is different then update the rest of values on the
                //frame.
                if (Double.parseDouble(initialPhiCos) != finalPhiCos)
                {
                   //If the method return true, then the range is correct, it is possible to update the frame.
                   if(PhiCosAction(initialPhiCos,finalPhiCos))
                   {
                      return true; //Return true if the checking is correct, then gives the focus.
                   }
                   else //If it is false re-write the initial voltage and re-assign the focus.
                   {
                       setPhiCos(initialPhiCos);
                       phiCosJTextField.requestFocus(); //Re-assign the focus to voltage text field
            ...
            {
                showErrorMessages("The phi cos value entered has not the correct format. The correct " +
                        "format is '#.#', example 1.0 or 0.85.");

                setPhiCos(initialPhiCos); //Re-write the initial value.
                phiCosJTextField.requestFocus(); //Re-assign the focus ta the text field.
            }
        ...
        //Adding the listeners for phi cos text field.

        //Changing the color of the background when the text field is focused. Yellow when is focused and white when
        //loses the focus. Calling to the method PhiCosAction() to know if the value of the field is changed.
        //If the value if changed, then update the frame.
        phiCosJTextField.addFocusListener(new FocusListener()
        {
            @Override
            public void focusGained(FocusEvent focusEvent)
            {
               phiCosJTextField.setBackground(Color.YELLOW);

               initialPhiCos=getPhiCos(); // Get the initial value of this field.
            }

            @Override
            public void focusLost(FocusEvent focusEvent)
            {
               phiCosJTextField.setText(String.valueOf(finalPhiCos));

               phiCosJTextField.setBackground(Color.WHITE);
            }
       }
       );//End of FocusListener() for voltage drop text field.

        //If the enter key is pressed then update the frame with the new value of the field.
        phiCosJTextField.addKeyListener(new KeyListener()
        {
             @Override
             public void keyTyped(KeyEvent keyEvent)
             {
                 ...
                 if(phiCosJTextField.getText().length() >= 3)
                 {
                     keyEvent.consume();
                 }
                 else
                 {
                     //Checking if there is written a point in the value.
                     if(points(phiCosJTextField.getText()) && (character == '.'))
                     {
                         keyEvent.consume(); //Ignore the character entered.
                     }
                     //Checking if the character is: 0123456789.
        ...
        VoltageDropPercentageJLabel.setBounds(new Rectangle(30,232,220,20));
        ...
        voltageDropPercentageJTextField.setBounds(new Rectangle(250,232,40,20));
        ...
        add(phiCosJLabel);
        add(phiCosJTextField);
        add(VoltageDropPercentageJLabel);
        add(voltageDropPercentageJTextField);
    }
    //End of the constructor JPanelMaterial_VoltageDropAC().
    ...
    /**
     * Getter method to get the phi cos value.
     * @return : Return the string with the value of the cos phi.
     */
    public String getPhiCos()
    {
        return phiCosJTextField.getText();
    }
     /**
    ...
    /**
     * Setter method to set the phi cos value.
     * @param PhiCos : String with the phi cos value.
     */
    public void setPhiCos(String PhiCos)
    {
        phiCosJTextField.setText(PhiCos);
    }
    ...
    public void paintComponent(Graphics graphics)
    {
        ...
        File imageFile=new File("src/Images/Wire.gif");
        ...
        {
            e.printStackTrace();
            System.out.println("The wire image does not exist.");
        }

        //Painting the image over the panel.
        graphics.drawImage(image, 50, 20, 250, 165, null);
    }
    ...
    {
        double voltagePowerSupply= Double.parseDouble(frame.getJPanelPowerSupplyAC().getVoltage());
        ...
        if (finalVoltageDrop <= 0 || finalVoltageDrop >= voltagePowerSupply)
        {
            showErrorMessages("The voltage drop value is out of range. The correct value must be" +
                    " between 0 and the voltage of the power supply: "+frame.getJPanelPowerSupplyAC().getVoltage()+
                    "VDC.");
            setVoltageDrop(initialVoltageDrop);
        ...
        {
            VoltageDropControllerAC voltageDropControllerAC = new VoltageDropControllerAC(frame);
            voltageDropControllerAC.updateFrame();
    ...
    /**
     * Method to compare the initial and the final value of the phi cos field. If the value is different, then
     * updated the frame.
     * @param initialPhiCos : String with the initial value of the phi cos field.
     * @param finalPhiCos : String with the final value of the phi cos field.
     * @return : True if the  range is correct and false if the value is out of the range.
     */
    private Boolean PhiCosAction(String initialPhiCos, Double finalPhiCos)
    {
        if (finalPhiCos < 0.8 || finalPhiCos > 1)
        {
            showErrorMessages("The phi cos value is out of range. The correct value must be 0.8 to 1");
            setPhiCos(initialPhiCos);
            return false;
        }
        else
        {
            PhiCosControllerAC phiCosControllerAC = new PhiCosControllerAC(frame);
            phiCosControllerAC.updateFrame();
            return true;
        }
    }
    //End of VoltageDropAction() method.
...
//End of the class JPanelMaterial_VoltageDropAC() class.
